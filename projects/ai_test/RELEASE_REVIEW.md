...
- HOLD: the completed source/license and dependency-inventory work is published through `ce929cd`. The existing rewrite candidate and exact-SHA lease dry run cover the older `31c5aa6` HEAD only, so they are stale and must be regenerated and reverified before any replacement push. The public API confirms the repository is public with one fork and one open pull request. The real rewrite push was not performed: collaborator/protection checks require authenticated access, and fork/PR coordination plus explicit public-default-branch force-push approval remain open. Complete dependency-license/vulnerability review, human provenance review, Windows MFC compilation, and other release checks also remain open.
