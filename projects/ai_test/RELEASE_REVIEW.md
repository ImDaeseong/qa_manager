...
## 공급망 라이선스
- 근거: [`LICENSE_SCOPE_REVIEW.md`](LICENSE_SCOPE_REVIEW.md)에 따라 `ai_test@31c5aa6`이 직접 작성 코드와 문서에 MIT(`Copyright (c) 2026 ImDaeseong`)를 적용했다. `THIRD_PARTY_NOTICES.md`는 JsonCpp 고지를 보존하고 Remotion·설치 의존성·외부 서비스가 루트 MIT 범위가 아님을 명시한다. 라이선스 가드는 Git 추적·원문 해시·필수 경계·README 링크를 검사하고 이유별 음성 테스트를 갖춘다. `DEPENDENCY_INVENTORY.md`는 추적 중인 모든 manifest의 범위·재현성 공백·fingerprint를 기록하며 기본/회귀 가드가 누락·잔존·변경 manifest와 npm lock 결함을 거부한다. 남은 검토: 각 의존성의 실제 라이선스·고지와 시점별 취약점을 검증한 완전한 SBOM, 과거 Git 이력, 사람의 권리·출시 판단을 완료한다.
...
- `ai_test@ce929cd`: added a tracked dependency inventory covering every manifest and build manifest, with normalized fingerprints and regression checks for missing, stale, changed, invalid, or unsupported manifest/lock evidence. Root tests report 17 passed plus 3 subtests; the qa_manager checklist reports 19/19 live checks passing.
- `ai_test@7afa58c`: made the license CRLF regression fixture independent of the checkout's existing line endings after a fresh rewritten-history checkout exposed the prior LF-only assumption.
...
- HOLD: the completed source/license, dependency-inventory, and CRLF regression work is published through `7afa58c`. Fresh rewrite candidate `2ddd8313`, updated recovery bundle, 19/19 live checks, strict fsck, bounded-secret scan, and exact-SHA lease dry run pass. The public API still confirms one fork and one open pull request. The real rewrite push was not performed: collaborator/protection checks require authenticated access, and fork/PR coordination plus explicit public-default-branch force-push approval remain open. Complete dependency-license/vulnerability review, human provenance review, Windows MFC compilation, and other release checks also remain open.
