...
- Origin rewrite completed: after explicit owner approval, exact `force-with-lease` replaced public `main` `7afa58c→2ddd8313`. Post-write `ls-remote`, fresh clone tree, strict fsck, and 19/19 live checks pass; the original local clone is aligned to rewritten `main`. One public fork remains on old unrelated history; the raw pulls API confirms zero open PRs. Remaining HOLDs are complete dependency-license/vulnerability review, human provenance review, Windows MFC compilation, third-party fork risk, and the other human release areas.
