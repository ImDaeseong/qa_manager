...
- Origin rewrite completed: after explicit owner approval, exact `force-with-lease` replaced public `main` `7afa58c→2ddd8313`. Post-write `ls-remote`, fresh clone tree, strict fsck, and 19/19 live checks pass; the original local clone is aligned to rewritten `main`. One public fork and one open PR still existed immediately before the write, so old history may remain distributed and their owners may need coordination. Remaining HOLDs are complete dependency-license/vulnerability review, human provenance review, Windows MFC compilation, fork/PR coordination, and the other human release areas.
